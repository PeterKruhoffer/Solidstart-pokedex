...
export const DBPokemon = Schema.Struct({
  id: Schema.Number,
  name: Schema.String,
...
export type DBPokemon = Schema.Schema.Type<typeof DBPokemon>;

const pokemonClient = new PokemonClient();

class FetchPokemonByIdError extends Data.TaggedError(
  "FetchPokemonByIdError",
)<{}> {}

function fetchPokemonById(
  id: number,
): Effect.Effect<Pokemon, FetchPokemonByIdError> {
...
class SavePokemonToDBError extends Data.TaggedError(
  "SavePokemonToDBError",
)<{}> {}
function trySavePokemonToDB(pokemon: DBPokemon) {
  return Effect.tryPromise({
    try: () =>
      db.insert(pokemonTable).values({
        id: pokemon.id,
        name: pokemon.name,
        types: pokemon.types,
...
export async function savePokemonToDB(pokemon: DBPokemon) {
  await Effect.runPromise(trySavePokemonToDB(pokemon));
  return;
}

class GetAllPokemonFromDBError extends Data.TaggedError(
  "GetAllPokemonFromDBError",
)<{}> {}
...
const trySetupPokemonDB = Effect.gen(function* () {
  ...
  const encodedPokemon = Schema.encodeSync(DBPokemon)(newPokemon);
  yield* trySavePokemonToDB(encodedPokemon);
  yield* Console.log(`#${dexNumber} should be save to DB`);
  dexNumber++;
}).pipe(
  Effect.catchTags({
    FetchPokemonByIdError: (error) =>
      Effect.succeed(`[Fetch_Pokemon_By_Id_Error]: ${error.message}`),
    SavePokemonToDBError: (error) =>
      Effect.succeed(`[Save_Pokemon_To_DB_Error]: ${error.message}`),
  }),
);
...
class DeletePokemonFromDBError extends Data.TaggedError(
  "DeletePokemonFromDBError",
)<{}> {}
