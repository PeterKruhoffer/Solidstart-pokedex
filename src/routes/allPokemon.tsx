        ...
        <div class="col-span-12 w-full">
          ...
          <Switch>
            ...
            <Match
              when={
                query.isSuccess &&
                (filteredTypes() === undefined || filteredTypes()?.length === 0)
              }
            >
        ...
        {(poke) => (
          <div class="flex flex-col justify-center items-center gap-4 border">
              ...
              <For each={poke.types}>
                {(type) => (
                  <div
                    class="px-3 py-1"
                    style={{
                      "background-color": typeColors[type.toLowerCase()].bg,
                    }}
                  >
                    <p
                      class="text-lg"
                      style={{ color: typeColors[type.toLowerCase()].color }}
                    >
                      {type}
                    </p>
                  </div>
            ...
            <img
              src={poke.sprite}
              alt={poke.name}
              class="size-52 pixelated border"
            />
          </div>
        )}
      </For>
