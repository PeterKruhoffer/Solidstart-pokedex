...
export default function PokemonDetails() {
  ...
  return (
    <section class="grid grid-cols-12 gap-8 p-10">
      ...
      {!pokemonDetails() && <p>Loading...</p>}
      {(() => {
        const details = pokemonDetails();
        if (!details) return <p>Loading...</p>;
        return (
          <div class="col-span-12 flex flex-col items-center">
            <h2 class="text-3xl font-bold capitalize">{details.name}</h2>
            <p class="text-lg">ID: {details.id}</p>
            <img
              ...
              <h3 class="text-xl font-semibold mb-2">Types</h3>
               <ul class="flex gap-4 flex-wrap">
                 {details.types?.map((typeInfo) => {
                   ...
                   const badgeClass = typeColors[typeName as keyof typeof typeColors] || "bg-gray-300 text-black";
                   return (
                     <li class={`capitalize px-3 py-1 rounded font-semibold ${badgeClass}`}>{typeName}</li>
                   );
                 })}
               </ul>              <div class="mt-6">
                <h3 class="text-xl font-semibold mb-2">Physical Attributes</h3>
                <p>Height: {(details.height / 10).toFixed(1)} m</p>
                <p>Weight: {(details.weight / 10).toFixed(1)} kg</p>
              </div>
              <div class="mt-6">
